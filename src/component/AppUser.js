...
const AppUser = () => {
  ...
  const { LoadState, LoadCity, LoadArea, RegisData, RegisImage ,LoginUser} = useSelector(
...
// console.log("rrr",MemberLoginData.member_name)
  let arrayRegisData = [];
  Object.keys(RegisData).map((key, index) => {
   if(LoginUser.member_name===RegisData[key].registrationBy)
   {
  ...
  return (
    <>
      ...
      <h2 className="text-center  text-danger"> App User</h2>
